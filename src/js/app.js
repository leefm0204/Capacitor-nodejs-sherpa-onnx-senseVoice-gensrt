...
function showToast(message, type) {
  // Toast notifications disabled - use console.log instead
  // console.log(`[Toast ${type}] ${message}`);
}

/**
 * Show error toast message - DISABLED
...
/**
 * Show success toast message - DISABLED
...
/**
 * Clear progress update timer for a file
 * @param {string} filename - File name to clear timer for
 */
function clearProgressTimer(filename) {
  const timer = progressUpdateTimers.get(filename);
  if (timer) {
    clearTimeout(timer);
  ...
  if (data.fileName && pendingFileProcesses.has(data.fileName)) {
    ...
    switch (data.type) {
      case "file_complete":
        ...
        clearProgressTimer(data.fileName); // FIX: Clear pending timer
...
elements.stopProcessBtn.addEventListener("click", async () => {
  ...
  if (
    ...
    // FIX: Clear progress timer for cancelled file
    clearProgressTimer(currentProcessingFile);
...
elements.clearListBtn.addEventListener("click", async () => {
  ...
  // FIX: Clear all pending progress timers
  for (const filename of progressUpdateTimers.keys()) {
    clearProgressTimer(filename);
  }
  domCache.clear();
