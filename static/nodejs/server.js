...
async function transcribePcmWaveform(
    ...
    // Memory report caching - avoid calling getReport() on every chunk
    // This is a huge performance optimization for long files
    let lastMemCheckTime = 0;
    let cachedHeapUsage = 70; // Default conservative threshold
    const MEM_CHECK_INTERVAL = 1000; // Only check memory every 1 second
    ...
    if (chunkSource.type === 'stream' && chunkSource.inputFile) {
        // For MP4/TS/MKV/WebM files: write SRT next to the input file (without video extension)
        const baseName = chunkSource.inputFile.replace(/\.(ts|mts|m2ts|mp4|m4a|mkv|webm|avi|mov|flv|wav)$/i, '');
        outPath = baseName + '.srt';
    } else if (chunkSource.type === 'sync' && chunkSource.inputFile) {
        // For WAV files: write SRT next to the input file (without audio extension)
        const baseName = chunkSource.inputFile.replace(/\.(wav|mp3|flac|aac|ogg|m4a)$/i, '');
        outPath = baseName + '.srt';
    } else {
    ...
    const processPcmChunk = (float32) => {
        ...
        // Cache the memory check to avoid expensive V8 stats calls on every chunk
        const now = Date.now();
        if (now - lastMemCheckTime > MEM_CHECK_INTERVAL) {
            const memReport = memoryManager.getReport();
            cachedHeapUsage = memReport.current.heap.usagePercent;
            lastMemCheckTime = now;
        }
        const heapUsage = cachedHeapUsage;
    ...
    try {
        ...
        // Cleanup - FIX: Don't free objects that are in activeResources from the loop
        // to avoid double-free. The createXxx() functions add to activeResources,
        // so we either remove them before explicit free, or skip them in the loop.
        
        // Remove the specific objects from activeResources before freeing
        if (vad) activeResources.delete(vad);
        if (recognizer) activeResources.delete(recognizer);
        if (buffer) activeResources.delete(buffer);
        
        // Now safe to free explicitly
        safeFree(vad);
        safeFree(recognizer);
        safeFree(buffer);

        // Clear any remaining unexpected resources
